...
function SiapKeluarPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Siap Keluar" description="Siapkan barang (picking list) untuk transaksi status Booking." />
      {list.length === 0 ? (
        <Empty text="Tidak ada transaksi Booking yang perlu disiapkan." />
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {list.map((t) => <PickingCard key={t.id} t={t} customer={getCustomer(t.customerId)?.nama ?? "-"} onDone={updateTransaction} />)}
        </div>
      )}
    </div>
  );
...
function PickingCard({ t, customer, onDone }: { t: Transaction; customer: string; onDone: (t: Transaction) => void }) {
  const [lines, setLines] = React.useState(t.items.map((l) => ({ ...l, qty_disiapkan: l.qty, checklist: false })));
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{customer} · keluar {formatDate(t.tanggal_mulai)}</p>
        </div>
      ...
      <CardContent className="space-y-3">
        ...
        {lines.map((l, idx) => (
          <div key={l.itemId} className="flex items-center gap-3 rounded-lg border bg-card p-3">
            <Checkbox checked={l.checklist} onCheckedChange={(v) => setLines(lines.map((x, i) => i === idx ? { ...x, checklist: !!v } : x))} />
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-medium">{l.nama}</p>
              <p className="text-xs text-muted-foreground">Dibutuhkan: {l.qty} unit</p>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-xs text-muted-foreground">Disiapkan</span>
              <Input type="number" min={0} className="h-8 w-16" value={l.qty_disiapkan}
                onChange={(e) => setLines(lines.map((x, i) => i === idx ? { ...x, qty_disiapkan: +e.target.value } : x))} />
            </div>
          </div>
        ))}
...
function Empty({ text }: { text: string }) {
  return <div className="rounded-xl border border-dashed py-16 text-center text-sm text-muted-foreground">{text}</div>;
}
